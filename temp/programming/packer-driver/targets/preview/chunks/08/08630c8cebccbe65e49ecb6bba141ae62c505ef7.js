    ...
    execute: function () {
      ...
      _export("PersistenceManager", PersistenceManager = class PersistenceManager {
        ...
        static saveSettings(settings) {
          try {
            var dataString = JSON.stringify(settings);
        ...
        static loadSettings() {
          try {
            var dataString = wx.getStorageSync((_crd && Constants === void 0 ? (_reportPossibleCrUseOfConstants({
              error: Error()
            }), Constants) : Constants).StorageKeys.SETTINGS);

            if (dataString && typeof dataString === 'string') {
              var data = JSON.parse(dataString); // 使用 SettingsData.fromObject 来处理可能不完整的旧数据或格式问题

              var settings = (_crd && SettingsData === void 0 ? (_reportPossibleCrUseOfSettingsData({
                error: Error()
        ...
        static saveUnfinishedGame(progress) {
          try {
            ...
            var dataString = JSON.stringify(progress);
            wx.setStorageSync((_crd && Constants === void 0 ? (_reportPossibleCrUseOfConstants({
              error: Error()
        ...
        static loadUnfinishedGame() {
          try {
            ...
            if (dataString && typeof dataString === 'string') {
              var data = JSON.parse(dataString); // 这里可以添加更严格的类型检查，确保加载的数据结构符合 LocalUserProgress
              // 例如，检查必要的字段是否存在
