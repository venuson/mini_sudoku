    ...
    execute: function () {
      ...
      _export("default", SudokuCore = class SudokuCore {
        ...
        generate(difficulty, unique) {
          ...
          var isUnique = unique === undefined ? true : unique; // Explicit boolean check
          // Get a set of squares and all possible candidates for each square
          // Use BLANK_BOARD constant

          var candidates = this._get_candidates_map(this.BLANK_BOARD); // Check if candidates generation failed (shouldn't for blank board, but good practice)
          ...
          var shuffled_squares = this._shuffle(this.SQUARES); // SQUARES is guaranteed non-null here


          for (var square of shuffled_squares) {
            // If an assignment of a random choice causes a contradiction, give up and try again
            var candidatesForSquare = candidates[square];
            ...
            var rand_candidate_idx = this._rand_range(candidatesForSquare.length);

            var rand_candidate = candidatesForSquare[rand_candidate_idx]; // _assign returns false on contradiction

            var assignmentResult = this._assign(candidates, square, rand_candidate);
            ...
            if (single_candidates.length >= difficultyLevel && this._strip_dups(single_candidates).length >= 8) {
              ...
              for (var i = 0; i < this.SQUARES.length; i++) {
                var _sq = this.SQUARES[i];

                if (candidates[_sq].length === 1) {
                  board += candidates[_sq];
                  givens_idxs.push(i);
                } else {
              ...
              if (nr_givens > difficultyLevel) {
                var shuffled_givens_idxs = this._shuffle(givens_idxs);

                var boardChars = board.split(''); // Modifiable array

                for (var _i = 0; _i < nr_givens - difficultyLevel; ++_i) {
                  var target_index = shuffled_givens_idxs[_i]; // Index is already a number
        ...
        solve(board, reverse) {
          ...
          var candidates = this._get_candidates_map(board); // Check if initial propagation led to contradiction
          ...
          if (result) {
            var solution = ""; // Ensure SQUARES is available and iterate in order
        ...
        get_candidates(board) {
          ...
          var candidates_map = this._get_candidates_map(board); // If there's an error (contradiction), return null
          ...
          var rows = [];
          var cur_row = [];
          var i = 0; // Iterate in the correct square order
        ...
        _get_candidates_map(board) {
          ...
          var squares_values_map = this._get_square_vals_map(board); // Start by assigning every digit as a candidate to every square
        ...
        _search(candidates, reverse) {
          ...
          var max_nr_candidates = 0; // let max_candidates_square: string | null = null; // Not actually used
          ...
          var min_nr_candidates = 10; // More than max possible (9)

          var min_candidates_square = null;

          for (var _square2 of this.SQUARES) {
            var _nr_candidates = candidates[_square2].length;

            if (_nr_candidates < min_nr_candidates && _nr_candidates > 1) {
              min_nr_candidates = _nr_candidates;
              min_candidates_square = _square2;
            }
          ...
          var min_candidates = candidates[min_candidates_square]; // Safe due to check above

          var candidatesToTry = reverse ? min_candidates.split('').reverse() : min_candidates.split('');

          for (var val of candidatesToTry) {
            ...
            var assignmentResult = this._assign(candidates_copy, min_candidates_square, val);

            var candidates_next = this._search(assignmentResult, reverse); // Pass result of _assign
        ...
        _assign(candidates, square, val) {
          ...
          for (var other_val of other_vals) {
            ...
            var eliminateResult = this._eliminate(candidates, square, other_val);
        ...
        _eliminate(candidates, square, val) {
          ...
          if (nr_candidates === 1) {
            var target_val = candidates[square]; // The single remaining candidate

            var peers = this.SQUARE_PEERS_MAP[square]; // Peers guaranteed by initialization check

            if (!peers) return false; // Safety check

            for (var peer of peers) {
              var eliminateResult = this._eliminate(candidates, peer, target_val);
          ...
          var units = this.SQUARE_UNITS_MAP[square]; // Units guaranteed by initialization check
          ...
          for (var unit of units) {
            ...
            if (val_places.length === 1) {
              var target_square = val_places[0]; // Assign `val` to `target_square` (which involves eliminating other values)
              // Use _assign which handles the propagation.

              var assignResult = this._assign(candidates, target_square, val);
        ...
        _get_square_peers_map(squares, units_map) {
          ...
          for (var cur_square of squares) {
            ...
            var peer_set = new Set(); // Use Set for efficient uniqueness check
            // Look through the current square's units map...

            for (var cur_unit of cur_square_units) {
              for (var cur_unit_square of cur_unit) {
        ...
        _get_all_units(rows, cols) {
          ...
          for (var rs of row_squares) {
            for (var cs of col_squares) {
        ...
        _shuffle(seq) {
          ...
          var shuffled = [...seq]; // Create a copy
        ...
        _force_range(nr, max, min) {
          ...
          */
          var num = nr === undefined || nr === null ? 0 : nr;
