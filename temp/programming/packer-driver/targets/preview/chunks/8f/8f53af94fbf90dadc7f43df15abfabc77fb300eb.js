System.register(["__unresolved_0", "cc", "__unresolved_1"], function (_export, _context) {
   ...
   */
  function createEmptyBoardData(size) {
    if (size === void 0) {
      size = (_crd && Constants === void 0 ? (_reportPossibleCrUseOfConstants({
        error: Error()
      }), Constants) : Constants).GRID_SIZE;
    }

    var grid = [];
    var presetMask = []; // 可选：如果需要，也初始化 solution
  ...
  function cloneBoardData(boardData) {
    var newGrid = boardData.grid.map(row => [...row]); // 深拷贝二维数组

    var newPresetMask = boardData.presetMask.map(row => [...row]); // 深拷贝二维数组

    var newSolution = boardData.solution ? boardData.solution.map(row => [...row]) : undefined; // 深拷贝可选的 solution
