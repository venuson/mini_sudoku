      ...
      */
      ...
      _export("CloudService", CloudService = class CloudService {
        ...
        static initializeSimulatedData() {
          ...
          const defaultUser = (_crd && createEmptyUserAccount === void 0 ? (_reportPossibleCrUseOfcreateEmptyUserAccount({
            error: Error()
        ...
        static async authenticateUser(code) {
          if (this.isSimulatedMode) {
            ...
            if (!this.simulatedUserDatabase.has(this.simulatedUserId)) {
              isNewUser = true;
              const newUser = (_crd && createEmptyUserAccount === void 0 ? (_reportPossibleCrUseOfcreateEmptyUserAccount({
                error: Error()
              }), createEmptyUserAccount) : createEmptyUserAccount)(this.simulatedUserId);
              newUser.nickname = "模拟新玩家";
              this.simulatedUserDatabase.set(this.simulatedUserId, newUser);
              log('[CloudService (Sim)] 模拟新用户创建:', newUser);
            }

            log(`[CloudService (Sim)] 模拟认证成功: userId=${this.simulatedUserId}, isNewUser=${isNewUser}`); // 使用 Promise.resolve 保持异步接口

            return Promise.resolve({
              userId: this.simulatedUserId,
              isNewUser: isNewUser
            });
          } else {
            try {
              const result = await this.callCloudFunction(this.CLOUD_FUNC_AUTH, {
                code: code
        ...
        static async updateUserRecord(userId, difficulty, levelIndex, timeSeconds, completionTimestamp) {
          if (this.isSimulatedMode) {
            ...
            const key = (_crd && generateLevelRecordKey === void 0 ? (_reportPossibleCrUseOfgenerateLevelRecordKey({
              error: Error()
            ...
            try {
              const result = await this.callCloudFunction(this.CLOUD_FUNC_UPDATE_RECORD, {
        ...
        static async fetchUserRecords(userId) {
          if (this.isSimulatedMode) {
            ...
            try {
              ...
              const result = await this.callCloudFunction(this.CLOUD_FUNC_FETCH_RECORDS // { userId: userId } // 如果需要获取其他用户的记录（通常不这么做）
              );
        ...
        static async fetchGlobalRanking(limit = 100) {
          if (this.isSimulatedMode) {
            this.initializeSimulatedData();
            ...
            const ranks = [];
            let myRankData = undefined;
            let currentUserRank = -1;

            for (let i = 0; i < allUsers.length; i++) {
              const user = allUsers[i];
              const rank = i + 1; // 创建 RankingEntry
            ...
            try {
              const result = await this.callCloudFunction(this.CLOUD_FUNC_FETCH_RANKING, {
                limit: limit
              });
        ...
        static async fetchLevelData(difficulty, levelIndex) {
        ...
        static async fetchUserAccountFromDB(userId) {
          if (this.isSimulatedMode) {
            this.initializeSimulatedData();
            ...
            try {
              const db = wx.cloud.database();
              const res = await db.collection(this.DB_COLLECTION_USERS).doc(userId) // 直接用 openid 作为文档 ID
              .get();
        ...
        */
        ...
        static async upsertUserAccountToDB(userData) {
          if (this.isSimulatedMode) {
            this.initializeSimulatedData();
            ...
            if (this.simulatedUserDatabase.has(userId)) {
              // 更新现有用户
              const existingUser = this.simulatedUserDatabase.get(userId); // 合并数据 (简单覆盖)
            ...
            try {
              const db = wx.cloud.database();
              const userId = userData.userId; // 从 userData 中移除 userId，因为它通常是文档 ID

              const dataToUpdate = { ...userData
              };
              delete dataToUpdate.userId;
              const res = await db.collection(this.DB_COLLECTION_USERS).doc(userId).set({
