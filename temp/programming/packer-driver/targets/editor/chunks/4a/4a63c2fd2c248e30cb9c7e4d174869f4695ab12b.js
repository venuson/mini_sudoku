      ...
      _export("AdManager", AdManager = (_dec = ccclass('AdManager'), _dec2 = property({
        ...
        createBannerAd(adUnitId, containerNode) {
          ...
          try {
            ...
            let bannerLeft = (screenWidth - bannerWidth) / 2; // 如果提供了容器节点，尝试获取其在屏幕上的位置和尺寸来定位 Banner
            ...
            if (isValid(containerNode)) {
              ...
              if (uiTransform) {
                ...
                const nodeWidth = uiTransform.width * director.getScene().getScale().x; // Cocos 世界坐标系原点在左下角，微信屏幕坐标系原点在左上角
                // 转换 Y 坐标

                const screenY = screenHeight - worldPos.y; // 尝试将 Banner 放在容器节点下方
