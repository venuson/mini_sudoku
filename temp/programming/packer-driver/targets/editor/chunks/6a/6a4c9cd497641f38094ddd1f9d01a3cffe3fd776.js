    ...
    execute: function () {
      ...
      _export("InputManager", InputManager = (_dec = ccclass('InputManager'), _dec(_class = class InputManager extends Component {
        ...
        onNumberInput(num) {
          ...
          const col = this._selectedCol; // 再次检查是否为预设格子 (安全校验)
        ...
        onClearButtonClicked() {
          ...
          const row = this._selectedRow;
          const col = this._selectedCol; // 检查是否为预设格子
        ...
        checkCompletionAfterUpdate(row, col) {
          ...
          const boxIndex = Math.floor(row / (_crd && Constants === void 0 ? (_reportPossibleCrUseOfConstants({
            error: Error()
