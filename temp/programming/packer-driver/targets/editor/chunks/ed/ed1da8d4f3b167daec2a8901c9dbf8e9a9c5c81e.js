      ...
      _export("AccountManager", AccountManager = (_dec = ccclass('AccountManager'), _dec(_class = (_class2 = class AccountManager extends Component {
        ...
        async login(forceRefresh = false) {
          ...
          if (!sys.platform.toLowerCase().includes('wechat')) {
            ...
            this._userAccount = (_crd && createEmptyUserAccount === void 0 ? (_reportPossibleCrUseOfcreateEmptyUserAccount({
              error: Error()
            }), createEmptyUserAccount) : createEmptyUserAccount)(this._userId);
            director.emit((_crd && Constants === void 0 ? (_reportPossibleCrUseOfConstants({
              error: Error()
            }), Constants) : Constants).EventName.LOGIN_SUCCESS, this._userId); // 发送游客登录成功事件
          ...
          try {
            ...
            const authResult = await (_crd && CloudService === void 0 ? (_reportPossibleCrUseOfCloudService({
              error: Error()
            }), CloudService) : CloudService).authenticateUser(code);
            this._userId = authResult.userId;
            this._isNewUser = authResult.isNewUser;
            console.log(`[AccountManager] 云端认证成功, userId: ${this._userId}, isNewUser: ${this._isNewUser}`); // 3. (可选) 如果是新用户，可能需要创建基础账户信息 (云函数内部可能已处理)
            // if (this._isNewUser) {
            ...
            director.emit((_crd && Constants === void 0 ? (_reportPossibleCrUseOfConstants({
              error: Error()
            }), Constants) : Constants).EventName.LOGIN_SUCCESS, this._userId); // 发送登录成功事件
        ...
        */
        ...
        async saveLevelResult(difficulty, levelIndex, timeSeconds) {
          ...
          try {
            ...
            await (_crd && CloudService === void 0 ? (_reportPossibleCrUseOfCloudService({
              error: Error()
            }), CloudService) : CloudService).updateUserRecord(this._userId, difficulty, levelIndex, timeSeconds, completionTimestamp); // --- 更新本地缓存的用户账户信息 ---
            ...
            if (this._userAccount) {
              ...
              director.emit((_crd && Constants === void 0 ? (_reportPossibleCrUseOfConstants({
                error: Error()
              }), Constants) : Constants).EventName.USER_RECORDS_UPDATED, this.getAllLevelRecords());
            } else {
        ...
        async fetchUserAccountData() {
          ...
          try {
            ...
            const recordsMap = await (_crd && CloudService === void 0 ? (_reportPossibleCrUseOfCloudService({
              error: Error()
            }), CloudService) : CloudService).fetchUserRecords(this._userId); // 需要从其他地方获取 totalLevelsCompleted, nickname, avatarUrl 等信息
            // 或者 fetchUserRecords 云函数直接返回完整的 UserAccount 结构
            // 假设 fetchUserRecords 返回的就是记录 Map

            const account = (_crd && createEmptyUserAccount === void 0 ? (_reportPossibleCrUseOfcreateEmptyUserAccount({
              error: Error()
            }), createEmptyUserAccount) : createEmptyUserAccount)(this._userId);
